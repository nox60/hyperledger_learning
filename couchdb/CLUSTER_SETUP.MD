...
curl http://admin:password@192.168.88.128:5984/_membership
```
...
docker run --name my-couchdb \
   ...
   -e COUCHDB_USER=admin \
   -e COUCHDB_PASSWORD=password \
   -e ERL_FLAGS='-setcookie "brumbrum" -name m1@my-couchdb -kernel inet_dist_listen_min 9100 -kernel inet_dist_listen_max 9100' \
   -d couchdb:3.0
...
docker run --name my-couchdb2 \
   ...
   -e COUCHDB_USER=admin \
   -e COUCHDB_PASSWORD=password \
   -e ERL_FLAGS='-setcookie "brumbrum" -name m2@my-couchdb2 -kernel inet_dist_listen_min 9101 -kernel inet_dist_listen_max 9101' \
   -d couchdb:3.0
```
...
docker run --name my.couchdb \
   ...
   -p 4369:4369 \
   --add-host my.couchdb:192.168.16.70 \
   --add-host my.couchdb2:192.168.16.71 \
   -e COUCHDB_USER=admin \
   -e COUCHDB_PASSWORD=password \
   -e ERL_FLAGS='-setcookie "brumbrum" -name couchdb1@my.couchdb -kernel inet_dist_listen_min 9100 -kernel inet_dist_listen_max 9200' \
   -d couchdb:3.0
...
docker run --name my.couchdb2 \
   ...
   -e COUCHDB_USER=admin \
   -e COUCHDB_PASSWORD=password \
   -e ERL_FLAGS='-setcookie "brumbrum2" -name couchdb2@my.couchdb2 -kernel inet_dist_listen_min 9100 -kernel inet_dist_listen_max 9200' \
   -d couchdb:3.0
```
